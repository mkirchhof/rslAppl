...
# train rsl
dataSubset <- sample(nrow(data$train), round(0.2 * nrow(data$train)))
# rsl <- learnRules(rsl, data$train[dataSubset, ], data$trainActual[dataSubset, ], nRules = nRules,
#                   method = "noisyor", batchsize = 20, reg = "beta", lambda = 5, maxHours = 44,
#                   maxLabelsPerRule = 5, alphaReg = 0.1, betaReg = 0.5, regDecay = 0.98, cluster = cl)


# evaluate and save
# save(rsl, file = paste0(folds, "_rsl.RData"))
load(paste0(folds, "_rsl.RData"))
clusterSetRNGStream(cl, iseed=22122020 - 1)
set.seed(22122020 - 1)
