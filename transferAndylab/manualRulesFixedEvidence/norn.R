...
.beliefPropagation <- function(norn, input, maxit = 20, convThresh = 1e-5, 
  ...
  # See which classifs and labels have evidence (and thus are fixed)
  hasEvidenceC <- classifs %in% names(input)
  names(hasEvidenceC) <- classifs
  hasEvidenceL <- hasEvidenceC
  names(hasEvidenceL) <- labels
  ...
  for(i in seq(maxit)){
    ...
    newOwnBel <- ownBel
    for(n in c(labels)){
      # TODO Include if isFixed then no change
      if(!hasEvidenceL[n]){
  ...
  # Compute own beliefs
  for(n in c(classifs, rules, auxs)){
    ownBel[[n]] <- ownLambda[[n]] * ownPi[[n]]
    ownBel[[n]] <- ownBel[[n]] / sum(ownBel[[n]])
  }
